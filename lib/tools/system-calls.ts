...
export class SystemCalls {
	 ...
	 */
	async getSdkBinaryPath(binaryName: string) {
	 ...
	 *                 local file system.
	 */
	async getBinaryFromSdkRoot(binaryName) {
		let binaryLoc: string | null = null;
		binaryName = this.getBinaryNameForOS(binaryName);
		let binaryLocs = [
			path.resolve(this.adb.sdkRoot, "platform-tools", binaryName),
			path.resolve(this.adb.sdkRoot, "emulator", binaryName),
			path.resolve(this.adb.sdkRoot, "tools", binaryName),
			path.resolve(this.adb.sdkRoot, "tools", "bin", binaryName)
		];
		// get subpaths for currently installed build tool directories
		_.forEach(await getBuildToolsDirs(this.adb.sdkRoot),
		...
		if (binaryLoc === null) {
			throw new Error(`Could not find ${binaryName} in ${binaryLocs}. ` +
											`Do you have the Android SDK installed at '${this.adb.sdkRoot}'?`);
		}
	 ...
	 */
	async getBinaryFromPath(binaryName: string) {
		let binaryLoc = null;
	...
	async getConnectedDevices(): Promise<Device[]> {
		...
		try {
			let {stdout} = await exec(this.adb.executable.path, this.adb.executable.defaultArgs.concat(['devices']));
	 ...
	 */
	async getDevicesWithRetry(timeoutMs = 20000) {
		let start = Date.now();
	 ...
	 */
	async restartAdb() {
		if (this.adb.suppressKillServer) {
	 ...
	 */
	async killServer() {
		log.debug(`Killing adb server on port ${this.adb.adbPort}`);
		await exec(this.adb.executable.path, [...this.adb.executable.defaultArgs, 'kill-server']);
	};
	 ...
	 */
	resetTelnetAuthToken = _.memoize(async function () {
	 ...
	 */
	async adbExecEmu(cmd) {
		// await this.verifyEmulatorConnected(); // todo
	...
	async adbExec(cmd, opts: any = {}) {
		...
		let execFunc = async () => {
			...
			try {
				...
				let args = this.adb.executable.defaultArgs.concat(cmd);
				log.debug(`Running '${this.adb.executable.path}' with args: ` +
									`${JSON.stringify(args)}`);
				let {stdout} = await exec(this.adb.executable.path, args, opts);
	 ...
	 * @throws {Error} If the command returned non-zero exit code.
	 */
	async shell(cmd: string[] | string, opts = {}) {
		if (!await this.adb.adbCommands.isDeviceConnected()) {
			throw new Error(`No device connected, cannot run adb shell command`);
		}
		let execCmd = ['shell'];
		if (cmd instanceof Array) {
	...
	createSubProcess(args: any[] = []) {
		// add the default arguments
		args = this.adb.executable.defaultArgs.concat(args);
		log.debug(`Creating ADB subprocess with args: ${JSON.stringify(args)}`);
		return new SubProcess(this.adb.adbCommands.getAdbPath(), args);
	}
	 ...
	 */
	async getAdbServerPort() {
		return this.adb.adbPort;
	}
	 ...
	 */
	getEmulatorPort = async function () {
	 ...
	 */
	getPortFromEmulatorString = function (emStr) {
	 ...
	 */
	setEmulatorPort = function (emPort) {
	...
	setDeviceId(deviceId: string) {
		...
		this.adb.curDeviceId = deviceId;
		let argsHasDevice = this.adb.executable.defaultArgs.indexOf('-s');
		if (argsHasDevice !== -1) {
			// remove the old device id from the arguments
			this.adb.executable.defaultArgs.splice(argsHasDevice, 2);
		}
		this.adb.executable.defaultArgs.push('-s', deviceId);
	}
	...
	async getRunningAVD(avdName) {
		try {
			...
			for (let emulator of emulators) {
				...
				let runningAVDName = await this.adb.adbCommands.sendTelnetCommand("avd name");
	 ...
	 */
	getRunningAVDWithRetry = async function (avdName, timeoutMs = 20000) {
		try {
			let start = Date.now();
	 ...
	 */
	killAllEmulators = async function () {
		let cmd, args;
		if (system.isWindows()) {
			cmd = 'TASKKILL';
	 ...
	 */
	killEmulator = async function (avdName) {
	 ...
	 */
	launchAVD = async function (avdName, avdArgs, language, country,
	 ...
	 */
	getAdbVersion = _.memoize(async function () {
		try {
	 ...
	 */
	checkAvdExist = async function (avdName) {
		let cmd, result;
		try {
	 ...
	 */
	waitForEmulatorReady = async function (timeoutMs = 20000) {
		let start = Date.now();
	 ...
	 */
	waitForDevice = async function (appDeviceReadyTimeout = 30) {
	 ...
	 */
	reboot = async function (retries = DEFAULT_ADB_REBOOT_RETRIES) {
		try {
			try {
	 ...
	 */
	fileExists = async function (remotePath) {
	 ...
	 *                          does not exist.
	 */
	async ls(remotePath, opts: string[] = []) {
		try {
	 ...
	 */
	async fileSize(remotePath: string) {
		try {
